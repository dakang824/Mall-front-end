  ...
  export default {
    ...
    methods: {
      handMove(e) {
        ...
        let backgroundX =
          objX * (Math.ceil(this.bigImg.width / this.imgObj.offsetWidth) / 1.5);
        let backgroundY =
          objY *
          (Math.ceil(this.bigImg.height / this.imgObj.offsetHeight) / 1.5);

        //判断是否超出界限
        let _maskX = objX - this.mouseMask.offsetHeight / 2;
        let _maskY = objY - this.mouseMask.offsetWidth / 2 - scrollTop;
      ...
      handOver(e) {
        if (!document.getElementById("_magnifier_layer")) {
          ...
          imgLayer.style.left = this.imgRect.width + "px";
          imgLayer.style.top = 0;
          imgLayer.style.backgroundImage = `url('${this.bigsrc}')`;
          imgLayer.style.backgroundRepeat = "no-repeat";
          document.getElementById("magnifier").appendChild(imgLayer);
        }
      },
    },
  };
